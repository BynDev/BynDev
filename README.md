...
**변재현, 프로덕트 방향성과 실행을 아우르는 PM & 개발자**   
**A PM & Developer Bridging Product Vision and Execution!**  
...
저는 아트, 프로그래밍, 기획 등 다양한 직군과 유연하게 협업하며, 각 파트의 관점을 이해하고 하나의 경험으로 조율합니다. 과정의 회고와 지식 자산화를 중시하며, 기술과 사용자 경험을 균형 있게 탐구하고 적용하는 역량을 갖추고 있습니다.
...
- **소개:** 아이들이 파스텔 톤 동화 이야기 속 세계에서 미션을 수행하며, AI가 ADHD 증상을 탐지하고 행동 발달 콘텐츠를 실시간으로 맞춤 제공하는 몰입형 AR 게임. **[UX]** 8–12세 아동 대상 스토리텔링, 아트 디렉션, 인터랙션 및 사용자 플로우 설계 **[AI 시스템]** 의료 AI 모델 구축, ONNX 변환 후 게임 엔진에 실시간 연동 **[게임 메커닉]** DSM-5 진단 기준을 게임 시나리오로 변환하여 행동 데이터를 수집하고 중앙 DB 기반 전처리 및 로그 전체 파이프라인 구현
...
- **소개:** 3D 뉴럴 렌더링 + 360° 영상 스트리밍을 활용해 원격 사용자 간 협업 공간 공유와 상호작용 가능성을 탐구하는 연구 프로젝트. **[연구 설계]** 선행 연구 리뷰, 연구 공백 확인, 3DGS 기반 공간 탐색 및 시각적 상호작용을 위한 디자인 스페이스 설계 **[개발]** Unity·HLSL 기반 게임 엔진과 Python·CUDA 파이프라인을 연동해 360° 스트리밍, 영상 데이터 수집 및 원격 3D Gaussian Splatting 재구성 자동화, 렌더링 정보 기반 인터랙션 기법 구현 **[사용자 실험]** N=24 사용자 실험을 통해 렌더링 지연과 사용자 신뢰도 관계 분석, 시점 인터랙션 기법 간 차이 통계적 비교
...
- **역할:** 연구 리드 & 게임 클라이언트 개발 [1저자] | **팀 구성**: 클라이언트 1 · 백엔드 1 · 데이터 분석 1
- **소개:** 실시간 심박 데이터를 이용해 VR 노출 치료에서 햅틱 피드백이 불안 완화에 미치는 효과를 연구. **[연구 설계]** 임상 이완 방법 검토 및 선행 연구와 행동 치료 요법 기반 3가지 햅틱 패턴 (느린 진동, 호흡 가이드, 버터플라이 허그) 설계 **[개발]** Galaxy Watch → WebSocket → Unity → bhaptics 이종 기술 스택 간 파이프라인 구축 및 타임스탬프 동기화 기반 로그 시스템 구현 **[사용자 실험]** 3가지 조건 비교 및 데이터 분석, 호흡 가이드 햅틱이 가장 효과적
