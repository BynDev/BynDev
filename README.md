...
- **소개:** 3D 뉴럴 렌더링 + 360° 영상 스트리밍을 활용해 원격 사용자 간 협업 공간 공유와 상호작용 가능성을 탐구하는 연구 프로젝트. **[연구 설계]** 선행 연구 리뷰, 연구 공백 확인, 3DGS 기반 공간 탐색 및 시각적 상호작용을 위한 디자인 스페이스 구축 **[개발]** Unity·HLSL 기반 게임 엔진과 Python·CUDA 파이프라인을 연동해 360° 스트리밍, 원격 3D Gaussian Splatting 재구성, 렌더링 정보 기반 인터랙션 기법 구현 **[사용자 실험]** N=24 사용자 실험을 통해 렌더링 지연과 사용자 신뢰도 관계 분석, 시점 인터랙션 기법 간 차이 통계적 비교
...
- **소개:** 실시간 심박 데이터를 이용해 VR 노출 치료에서 햅틱 피드백이 불안 완화에 미치는 효과를 연구 **[연구 설계]** 임상 이완 방법 검토 및 선행 연구와 행동 치료 요법 기반 3가지 햅틱 패턴 설계 (느린 진동, 호흡 가이드, 버터플라이 허그) **[개발]** Galaxy Watch → WebSocket → Unity → bhaptics 이종 기술 스택 간 파이프라인 구축, 타임스탬프 동기화 기반 로그 시스템 구현 **[사용자 실험]** 3가지 조건 비교 및 데이터 분석, 호흡 가이드 햅틱이 가장 효과적
