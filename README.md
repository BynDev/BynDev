...
- **소개:** 아이들이 파스텔 톤 동화 이야기 속 세계에서 미션을 수행하며, AI가 ADHD 증상을 탐지하고 행동 발달 콘텐츠를 실시간으로 맞춤 제공하는 몰입형 AR 게임. **[설계]** 8–12세 아동을 대상으로 한 스토리텔링, 아트 디렉션, DSM-5 진단 기준을 게임 시나리오로 구조화하고 인터랙션 및 사용자 플로우 설계 **[AI 시스템]** 의료 AI 모델 구축, ONNX 변환 후 게임 엔진에 실시간 연동 **[백엔드]** 사용자 행동 및 플레이 데이터를 AI 활용이 가능한 형태(AI-Ready Data)로 정제·모델링하고 중앙 DB 기반의 전처리 및 로그 수집·처리 파이프라인을 구축
